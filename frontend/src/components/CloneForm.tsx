...
import { CloneJob, CloneRequest, ErrorResponse } from '../app/types';
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch('http://localhost:8000/clone', {
        ...
        body: JSON.stringify({ url } as CloneRequest),
      });

      if (!response.ok) {
        const errorData = await response.json() as ErrorResponse;
        throw new Error(errorData.detail || 'Failed to start cloning job');
      }

      const job = await response.json() as CloneJob;
      onJobStart(job);
      setUrl('');
    } catch (error) {
